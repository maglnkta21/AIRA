...
# --- Data untuk MODEL (numerik) ---
model_data = {
    "Gender": 1 if gender == "Male" else 2,
    "Age": age,
    "Smoking": encode_yesno_dataset(smoking),
    "Yellow Fingers": encode_yesno_dataset(bool_to_yesno(yellow)),
    "Anxiety": encode_yesno_dataset(anxiety),
    "Peer Pressure": encode_yesno_dataset(peer),
    "Chronic Disease": encode_yesno_dataset(bool_to_yesno(chronic)),
    "Fatigue ": encode_yesno_dataset(bool_to_yesno(fatigue)),
    "Allergy ": encode_yesno_dataset(bool_to_yesno(allergy)),
    "Wheezing": encode_yesno_dataset(bool_to_yesno(wheezing)),
    "Alcohol Consuming": encode_yesno_dataset(alcohol),
    "Coughing": encode_yesno_dataset(bool_to_yesno(coughing)),
    "Shortness of Breath": encode_yesno_dataset(bool_to_yesno(shortness)),
    "Swallowing Difficulty": encode_yesno_dataset(bool_to_yesno(swallowing)),
    "Chest Pain": encode_yesno_dataset(bool_to_yesno(chest_pain))
}

df_model = pd.DataFrame([model_data])

# --- Data untuk DISPLAY (YES / NO) ---
display_data = {
    "Gender": gender,
    "Age": age,
    "Smoking": smoking,
    "Yellow Fingers": bool_to_yesno(yellow),
    "Anxiety": anxiety,
    "Peer Pressure": peer,
    "Chronic Disease": bool_to_yesno(chronic),
    "Fatigue": bool_to_yesno(fatigue),
    "Allergy": bool_to_yesno(allergy),
    "Wheezing": bool_to_yesno(wheezing),
    "Alcohol Consuming": alcohol,
    "Coughing": bool_to_yesno(coughing),
    "Shortness of Breath": bool_to_yesno(shortness),
    "Swallowing Difficulty": bool_to_yesno(swallowing),
    "Chest Pain": bool_to_yesno(chest_pain)
}
...
if st.button("🔍 Prediksi Risiko"):
    try:
        pred_class = model_ai.predict(df_model)[0]
        proba = model_ai.predict_proba(df_model)[0]
